...
def wandb_init(cfg: DictConfig, jobname, tagslist, notes=None):
    wandb.init(
        project='Partially Matched Trajectories Heatmap Exps',
        name=jobname,
        notes=notes,
        tags=tagslist,
        config=cfg
...
def main(cfg: DictConfig):
    ...
    if cfg.data_files.use_wandb:
        wandb_init(cfg_raw, jobname, ["rl_Apr", f'task_id-{cfg.rl_data_files.rl_task}', 'test_false_positive'])
    ...
    while True:
        ...
        for _ in range(cfg.rl_params.num_step):
            ...
            no_achieve_state_constraints_arr, no_achieve_action_constraints_arr = [], []
            walkthrough_accu_rew_one_arr = []
            walkthrough_accu_rew_two_arr = []
            walkthrough_accu_rew_three_arr = []
            walkthrough_accu_rew_four_arr = []
            walkthrough_accu_rew_five_arr = []



            for parent_conn in parent_conns:
                next_state, reward, done, real_done, log_reward, otherinfo = parent_conn.recv()
                no_ofwins, no_achieve_state_constraints, no_achieve_action_constraints, walkthrough_accu_rew = otherinfo[:4]
                # save agent pos info to create heatmap
                agent_pos = otherinfo[-2]
                agent_pos_save_arr.append(agent_pos)
                ...
                no_achieve_state_constraints_arr.append(no_achieve_state_constraints)
                no_achieve_action_constraints_arr.append(no_achieve_action_constraints)
                walkthrough_len = len(walkthrough_accu_rew)
                walkthrough_accu_rew_one_arr.append(walkthrough_accu_rew[0])
                walkthrough_accu_rew_two_arr.append(walkthrough_accu_rew[1])
                walkthrough_accu_rew_three_arr.append(walkthrough_accu_rew[2])
                if walkthrough_len > 3:
                    walkthrough_accu_rew_four_arr.append(walkthrough_accu_rew[3])
                    walkthrough_accu_rew_five_arr.append(walkthrough_accu_rew[4])
                # -----------------------
                next_obs.append(next_state[3, :, :].reshape([1, 84, 84]))
            ...
            if len(agent_pos_save_arr) > 5000000:
                with open(os.path.join(get_original_cwd(), f"agent_pos_save_{agent_pos_save_arr_count}.pkl"), 'wb') as f:
                    pickle.dump(agent_pos_save_arr, f)
                agent_pos_save_arr_count += 1
                agent_pos_save_arr = []

            # ----- log stuffs ------
            no_of_wins_arr = np.hstack(no_of_wins_arr)
            no_achieve_state_constraints_arr = np.hstack(no_achieve_state_constraints_arr)
            no_achieve_action_constraints_arr = np.hstack(no_achieve_action_constraints_arr)
            walkthrough_accu_rew_one_arr = np.hstack(walkthrough_accu_rew_one_arr)
            walkthrough_accu_rew_two_arr = np.hstack(walkthrough_accu_rew_two_arr)
            walkthrough_accu_rew_three_arr = np.hstack(walkthrough_accu_rew_three_arr)
            if walkthrough_len > 3:
                walkthrough_accu_rew_four_arr = np.hstack(walkthrough_accu_rew_four_arr)
                walkthrough_accu_rew_five_arr = np.hstack(walkthrough_accu_rew_five_arr)
            ...
            if real_dones[sample_env_index]:
                ...
                no_achieve_state_constraints = no_achieve_state_constraints_arr[sample_env_index]
                no_achieve_action_constraints = no_achieve_action_constraints_arr[sample_env_index]
                walkthrough_accu_rew_one = walkthrough_accu_rew_one_arr[sample_env_index]
                walkthrough_accu_rew_two = walkthrough_accu_rew_two_arr[sample_env_index]
                walkthrough_accu_rew_three = walkthrough_accu_rew_three_arr[sample_env_index]
                if walkthrough_len > 3:
                    walkthrough_accu_rew_four = walkthrough_accu_rew_four_arr[sample_env_index]
                    walkthrough_accu_rew_five = walkthrough_accu_rew_five_arr[sample_env_index]
                else:
                    walkthrough_accu_rew_four = 0.0
                    walkthrough_accu_rew_five = 0.0
                    ...
                    wandb.log(
                        {
                            ...
                            "no_of_win": no_of_win,
                            "no_achieve_state_constraints": no_achieve_state_constraints,
                            "no_achieve_action_constraints": no_achieve_action_constraints,
                            "walkthrough_accu_rew_one": walkthrough_accu_rew_one,
                            "walkthrough_accu_rew_two": walkthrough_accu_rew_two,
                            "walkthrough_accu_rew_three": walkthrough_accu_rew_three,
                            "walkthrough_accu_rew_four": walkthrough_accu_rew_four,
                            "walkthrough_accu_rew_five": walkthrough_accu_rew_five,
                        }
                    )
